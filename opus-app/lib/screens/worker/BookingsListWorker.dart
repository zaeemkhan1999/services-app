...
class BookingsListWorker extends StatefulWidget {
  @override
  _BookingsListWorkerState createState() => _BookingsListWorkerState();
}

class _BookingsListWorkerState extends State<BookingsListWorker> {
  ...
  getAllWorkers() async {
    var resp = await ApiHandler().makeGetRequest("/worker");
    print(resp);
    ...
    this.workers.forEach((worker) async {
      ...
      this.serviceProviders.add(ServiceProviders(
            ...
            rating:
                double.parse(worker["ServiceDetails"]["BaseRate"].toString()),
  ...
  Widget _cardDesign(ServiceProviders) {
    return InkWell(
      onTap: () {
